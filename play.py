...
that card up (only cards you can legally take are highlighted). Discard phase:
click a card in your hand. Keys: A toggles the advisor (the search's expected
outcome for each of your legal moves), N starts a new game, Q quits.
"""
import argparse
import os
import sys
...
class View:
    ...
    def draw(self, game):
        ...
        if human_turn and not discard and legal[0]:
            self.hit.append((rect, 0))
        pile = g.pile()
        x = 140
        self.text("Discard pile (oldest -> newest)", x, y - 44, self.small, DIM)
        ...
        if human_turn and not discard and 0 in g.advice:
            self.text(f"deck {g.advice[0][0]:+.0f}", 20, y - 44, self.small, HIGHLIGHT)

        # Human hand (bottom)
        hand = g.hand()
        self.text(f"You  -  {len(hand)} cards, score {g.env.get_score(g.human):.0f}", 20, 520, self.big)
        x = 20
        ...
        for card in hand:
            ...
            rect = self.card(int(card), x, 555, highlight=ok, label=label)
            if ok:
                self.hit.append((rect, action))
            x += step
        ...
        if g.result:
            ...
            status = "Your turn: " + ("choose a card to discard." if discard else "click the deck or a pile card you can take.")
        else:
    ...
    def click(self, pos):
        for rect, action in reversed(self.hit):   # topmost card wins
