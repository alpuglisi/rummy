...
def env_worker(remote, parent_remote, seed):
    """
    Worker process that runs the C++ environment on an independent CPU core.
    """
    parent_remote.close()
    ...
    while True:
        ...
        if cmd == 'step':
            ...
            if done:
                env.reset() # Auto-reset on game over for continuous rolling
...
class VectorizedRummyEnv:
    """
    Manager class to step multiple C++ environments concurrently.
    """
    def __init__(self, num_envs):
        ...
        for i, (work_remote, remote) in enumerate(zip(self.work_remotes, self.remotes)):
            # Ensure unique seeds across all environment workers
